...
static void on_process(void* userdata) {
  ...
  printf("got a frame of size %d and height %d\n",
         ...
         data->format.info.mjpg.size.height);

  if (buf->datas[0].type == SPA_DATA_MemFd ||
      buf->datas[0].type == SPA_DATA_DmaBuf) {
    map = (uint8_t*)mmap(NULL,
                         buf->datas[0].maxsize + buf->datas[0].mapoffset,
                         PROT_READ,
                         MAP_PRIVATE,
                         buf->datas[0].fd,
                         0);
  ...
  uint8_t* dst_y = new uint8_t[data->format.info.mjpg.size.width *
                               data->format.info.mjpg.size.height];
  uint8_t* dst_u = new uint8_t[(data->format.info.mjpg.size.width *
                                    data->format.info.mjpg.size.height +
                                3) /
                               4];
  uint8_t* dst_v = new uint8_t[(data->format.info.mjpg.size.width *
                                    data->format.info.mjpg.size.height +
                                3) /
                               4];

  int ret = libyuv::ConvertToI420(sdata,
                                  buf->datas[0].chunk->size,
                                  dst_y,
                                  data->format.info.mjpg.size.width,
                                  dst_u,
                                  (data->format.info.mjpg.size.width + 1) / 2,
                                  dst_v,
                                  (data->format.info.mjpg.size.width + 1) / 2,
                                  0,
                                  0,
...
static void on_param_changed(void* userdata,
                             uint32_t id,
  ...
  if (spa_format_video_mjpg_parse(param, &data->format.info.mjpg) < 0) return;
}
...
int CaptureFrames() {
  ...
  {
    std::scoped_lock lock{loop_mutex};
    data.loop = loop;
    ...
    params[0] = (spa_pod*)spa_pod_builder_add_object(
        ...
        SPA_POD_Id(SPA_MEDIA_SUBTYPE_mjpg));
